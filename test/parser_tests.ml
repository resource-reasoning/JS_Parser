...
let test_var () =
  ...
  assert_equal_exp (add_script (mk_exp (VarDec ["x", None]) 0)) exp

let test_var_value () =
  let exp = exp_from_string "var x = 5" in
  let num_5 = mk_exp (Num 5.0) 8 in
  assert_equal_exp (add_script(mk_exp (VarDec ["x", Some num_5]) 0)) exp
...
let test_var_list () =
  ...
  assert_equal_exp (add_script vardec) exp

let test_regexp () =
  let exp = exp_from_string "/^\\s+/" in
  assert_equal_exp (add_script (mk_exp (RegExp ("^\\s+", "")) 0)) exp

let test_regexp_with_flags () =
  let exp = exp_from_string "/^\\s+/g" in
  assert_equal_exp (add_script (mk_exp (RegExp ("^\\s+", "g")) 0)) exp

let test_not () =
  let exp = exp_from_string "!selector" in
  let selector = mk_exp (Var "selector") 1 in
  assert_equal_exp (add_script (mk_exp (Unary_op (Not, selector)) 0)) exp

let test_caccess () =
  let exp = exp_from_string "this[0]" in
  let this = mk_exp This 0 in
  let zero = mk_exp (Num 0.0) 5 in
  assert_equal_exp (add_script (mk_exp (CAccess (this, zero)) 0)) exp

let test_and () =
  let exp = exp_from_string "a && b" in
  let a = mk_exp (Var "a") 0 in
  let b = mk_exp (Var "b") 5 in
  assert_equal_exp (add_script (mk_exp (BinOp (a, Boolean And, b)) 0)) exp

let test_array_literal () =
  let exp = exp_from_string "[,x,,y]" in
  let x = mk_exp (Var "x") 2 in 
  let y = mk_exp (Var "y") 5 in
  assert_equal_exp (add_script (mk_exp (Array [None; Some x; None; Some y]) 0)) exp

let test_ge () =
  let exp = exp_from_string "1 >= 2" in
  let one = mk_exp (Num 1.0) 0 in
  let two = mk_exp (Num 2.0) 5 in
  assert_equal_exp (add_script (mk_exp (BinOp (one, Comparison Ge, two)) 0)) exp

let test_or () =
  let exp = exp_from_string "a || b" in
  let a = mk_exp (Var "a") 0 in
  let b = mk_exp (Var "b") 5 in
  assert_equal_exp (add_script (mk_exp (BinOp (a, Boolean Or, b)) 0)) exp

let test_not_triple_eq () =
  let exp = exp_from_string "a !== b" in
  let a = mk_exp (Var "a") 0 in
  let b = mk_exp (Var "b") 6 in
  assert_equal_exp (add_script (mk_exp (BinOp (a, Comparison NotTripleEqual, b)) 0)) exp
...
let test_hook () =
  ...
  assert_equal_exp (add_script (mk_exp (ConditionalOp (ab, a9, b13)) 0)) exp

let test_instanceof () =
  let exp = exp_from_string "a instanceof b" in
  let a = mk_exp (Var "a") 0 in
  let b = mk_exp (Var "b") 13 in
  assert_equal_exp (add_script (mk_exp (BinOp (a, Comparison InstanceOf, b)) 0)) exp

let test_typeof () =
  let exp = exp_from_string "typeof selector" in
  let selector = mk_exp (Var "selector") 7 in
  assert_equal_exp (add_script (mk_exp (Unary_op (TypeOf, selector)) 0)) exp
...
let test_pos () =
  ...
  assert_equal_exp (add_script (mk_exp (Unary_op (Positive, a1)) 0)) exp

let test_dec_pre () =
  let exp = exp_from_string "--a" in
  let a = mk_exp (Var "a") 2 in
  assert_equal_exp (add_script (mk_exp (Unary_op (Pre_Decr, a)) 0)) exp

let test_dec_post () =
  let exp = exp_from_string "a--" in
  let a = mk_exp (Var "a") 0 in
  assert_equal_exp (add_script (mk_exp (Unary_op (Post_Decr, a)) 0)) exp

let test_inc_pre () =
  let exp = exp_from_string "++a" in
  let a = mk_exp (Var "a") 2 in
  assert_equal_exp (add_script (mk_exp (Unary_op (Pre_Incr, a)) 0)) exp

let test_inc_post () =
  let exp = exp_from_string "a++" in
  let a = mk_exp (Var "a") 0 in
  assert_equal_exp (add_script (mk_exp (Unary_op (Post_Incr, a)) 0)) exp
...
let test_for () =
  ...
  in assert_equal_exp (add_script loop) exp
...
let test_forin () =
  ...
  assert_equal_exp (add_script (mk_exp (ForIn (varprop, oldObj1, block)) 0)) exp

let test_assign_add () =
  let exp = exp_from_string "a += b" in
  let a = mk_exp (Var "a") 0 in
  let b = mk_exp (Var "b") 5 in
  assert_equal_exp (add_script (mk_exp (AssignOp (a, Plus, b)) 0)) exp

let test_assign_sub () =
  let exp = exp_from_string "a -= b" in
  let a = mk_exp (Var "a") 0 in
  let b = mk_exp (Var "b") 5 in
  assert_equal_exp (add_script (mk_exp (AssignOp (a, Minus, b)) 0)) exp

let test_assign_mul () =
  let exp = exp_from_string "a *= b" in
  let a = mk_exp (Var "a") 0 in
  let b = mk_exp (Var "b") 5 in
  assert_equal_exp (add_script (mk_exp (AssignOp (a, Times, b)) 0)) exp

let test_assign_div () =
  let exp = exp_from_string "a /= b" in
  let a = mk_exp (Var "a") 0 in
  let b = mk_exp (Var "b") 5 in
  assert_equal_exp (add_script (mk_exp (AssignOp (a, Div, b)) 0)) exp

let test_assign_mod () =
  let exp = exp_from_string "a %= b" in
  let a = mk_exp (Var "a") 0 in
  let b = mk_exp (Var "b") 5 in
  assert_equal_exp (add_script (mk_exp (AssignOp (a, Mod, b)) 0)) exp

let test_assign_ursh () =
  let exp = exp_from_string "a >>>= b" in
  let a = mk_exp (Var "a") 0 in
  let b = mk_exp (Var "b") 7 in
  assert_equal_exp (add_script (mk_exp (AssignOp (a, Ursh, b)) 0)) exp

let test_assign_lsh () =
  let exp = exp_from_string "a <<= b" in
  let a = mk_exp (Var "a") 0 in
  let b = mk_exp (Var "b") 6 in
  assert_equal_exp (add_script (mk_exp (AssignOp (a, Lsh, b)) 0)) exp

let test_assign_rsh () =
  let exp = exp_from_string "a >>= b" in
  let a = mk_exp (Var "a") 0 in
  let b = mk_exp (Var "b") 6 in
  assert_equal_exp (add_script (mk_exp (AssignOp (a, Rsh, b)) 0)) exp

let test_assign_bitand () =
  let exp = exp_from_string "a &= b" in
  let a = mk_exp (Var "a") 0 in
  let b = mk_exp (Var "b") 5 in
  assert_equal_exp (add_script (mk_exp (AssignOp (a, Bitand, b)) 0)) exp

let test_assign_bitor () =
  let exp = exp_from_string "a |= b" in
  let a = mk_exp (Var "a") 0 in
  let b = mk_exp (Var "b") 5 in
  assert_equal_exp (add_script (mk_exp (AssignOp (a, Bitor, b)) 0)) exp

let test_assign_bitxor () =
  let exp = exp_from_string "a ^= b" in
  let a = mk_exp (Var "a") 0 in
  let b = mk_exp (Var "b") 5 in
  assert_equal_exp (add_script (mk_exp (AssignOp (a, Bitxor, b)) 0)) exp

let test_notequal () =
  let exp = exp_from_string "a != b" in
  let a = mk_exp (Var "a") 0 in
  let b = mk_exp (Var "b") 5 in
  assert_equal_exp (add_script (mk_exp (BinOp (a, Comparison NotEqual, b)) 0)) exp

let test_gt () =
  let exp = exp_from_string "a > b" in
  let a = mk_exp (Var "a") 0 in
  let b = mk_exp (Var "b") 4 in
  assert_equal_exp (add_script (mk_exp (BinOp (a, Comparison Gt, b)) 0)) exp

let test_in () =
  let exp = exp_from_string "a in b" in
  let a = mk_exp (Var "a") 0 in
  let b = mk_exp (Var "b") 5 in
  assert_equal_exp (add_script (mk_exp (BinOp (a, Comparison In, b)) 0)) exp

let test_comma1 () =
  let exp = exp_from_string "a , b" in
  let a = mk_exp (Var "a") 0 in
  let b = mk_exp (Var "b") 4 in
  assert_equal_exp (add_script (mk_exp (Comma (a, b)) 0)) exp
...
let test_comma2 () =
  ...
  assert_equal_exp (add_script (mk_exp (Comma (ab, c)) 0)) exp

let test_negative () =
  let exp = exp_from_string "-a" in
  let a = mk_exp (Var "a") 1 in
  assert_equal_exp (add_script (mk_exp (Unary_op (Negative, a)) 0)) exp

let test_bitnot () =
  let exp = exp_from_string "~a" in
  let a = mk_exp (Var "a") 1 in
  assert_equal_exp (add_script (mk_exp (Unary_op (Bitnot, a)) 0)) exp

let test_void () =
  let exp = exp_from_string "void a" in
  let a = mk_exp (Var "a") 5 in
  assert_equal_exp (add_script (mk_exp (Unary_op (Void, a)) 0)) exp

let test_mod () =
  let exp = exp_from_string "a % b" in
  let a = mk_exp (Var "a") 0 in
  let b = mk_exp (Var "b") 4 in
  assert_equal_exp (add_script (mk_exp (BinOp (a, Arith Mod, b)) 0)) exp

let test_ursh () =
  let exp = exp_from_string "a >>> b" in
  let a = mk_exp (Var "a") 0 in
  let b = mk_exp (Var "b") 6 in
  assert_equal_exp (add_script (mk_exp (BinOp (a, Arith Ursh, b)) 0)) exp

let test_lsh () =
  let exp = exp_from_string "a << b" in
  let a = mk_exp (Var "a") 0 in
  let b = mk_exp (Var "b") 5 in
  assert_equal_exp (add_script (mk_exp (BinOp (a, Arith Lsh, b)) 0)) exp

let test_rsh () =
  let exp = exp_from_string "a >> b" in
  let a = mk_exp (Var "a") 0 in
  let b = mk_exp (Var "b") 5 in
  assert_equal_exp (add_script (mk_exp (BinOp (a, Arith Rsh, b)) 0)) exp

let test_bitand () =
  let exp = exp_from_string "a & b" in
  let a = mk_exp (Var "a") 0 in
  let b = mk_exp (Var "b") 4 in
  assert_equal_exp (add_script (mk_exp (BinOp (a, Arith Bitand, b)) 0)) exp

let test_bitor () =
  let exp = exp_from_string "a | b" in
  let a = mk_exp (Var "a") 0 in
  let b = mk_exp (Var "b") 4 in
  assert_equal_exp (add_script (mk_exp (BinOp (a, Arith Bitor, b)) 0)) exp

let test_bitxor () =
  let exp = exp_from_string "a ^ b" in
  let a = mk_exp (Var "a") 0 in
  let b = mk_exp (Var "b") 4 in
  assert_equal_exp (add_script (mk_exp (BinOp (a, Arith Bitxor, b)) 0)) exp

let test_return () =
  let exp = exp_from_string "function f() {return}" in
  let r = mk_exp (Return None) 14 in
  let block = mk_exp (Block [r]) 13 in
  assert_equal_exp (add_script (mk_exp (NamedFun (false, "f", [], block)) 0)) exp
...
let test_delete () =
  ...
  let a = mk_exp (Var "a") 7 in
  assert_equal_exp (add_script (mk_exp (Delete a) 0)) exp
...
let test_continue () =
  ...
  in assert_equal_exp (add_script correct_exp) exp
...
let test_continue_label () =
  ...
  in assert_equal_exp (add_script (mk_exp (Label ("test", loop)) 0)) exp
...
let test_break () =
  ...
  let whilee =
    if supports_annots () then
      ...
      mk_exp (While (condition, body)) 0
  in assert_equal_exp (add_script whilee) exp
...
let test_try_catch () =
  ...
  assert_equal_exp (add_script (mk_exp (Try (ablock, Some ("b", cblock), None)) 0)) exp
...
let test_try_catch_finally () =
  ...
  assert_equal_exp (add_script (mk_exp (Try (ablock, Some ("b", cblock), Some dblock)) 0)) exp
...
let test_try_finally () =
  ...
  assert_equal_exp (add_script (mk_exp (Try (ablock, None, Some dblock)) 0)) exp
...
let test_switch () =
  ...
  assert_equal_exp (add_script (mk_exp (Switch (a, [(Case one, block1); (DefaultCase, block2); (Case two, block3)])) 0)) exp

let test_debugger () =
  let exp = exp_from_string "debugger" in
  assert_equal_exp (add_script (mk_exp Debugger 0)) exp

let test_top_annotations () =
  if supports_annots () then begin
    let exp = exp_from_string "/** @topensures #cScope = [#lg] */ debugger" in
    assert_equal_exp (mk_exp_with_annot (Script (false, [mk_exp Debugger 35])) 0 [{annot_type = TopEnsures; annot_formula = "#cScope = [#lg]"}]) exp
  end
...
let xml_parser () =
  use_parser_xml ~path:!js_parser ~verbose:false
...
let xml_suite = "Testing Closure (XML) Parser" >:::
  (("Initialise XML Parser" >:: xml_parser) :: Parser_xml_tests.tests @ shared_tests)

let json_suite = "Testing Esprima (JSON) Parser" >:::
  (("initialize json parser" >:: json_parser) :: Parser_json_tests.tests @ shared_tests)

let suite = "Testing Parser" >::: [xml_suite; json_suite]
...
let arguments () =
  ...
  Arg.parse
    [ "-jsparser",
      Arg.String(fun f -> js_parser := f), "path to js_parser.jar (optional)"
    ]
...
let _ =
  ...
  ELSE
    arguments () ;
    exit (exit_code (run_test_tt_main suite))
  END
