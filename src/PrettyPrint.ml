...
let string_of_bool_op x = match x with And -> "&&" | Or -> "||"
...
and string_of_exp_syntax_1 expstx with_annot =
  ...
  let fop e = match e with None -> "" | Some e -> f e in
  let string_op s = match s with None -> "" | Some str -> str in
  match expstx with
  ...
  | Unary_op (op, e) -> (
    ...
    | _ -> Printf.sprintf "%s %s" (string_of_unary_op op) (f e) )
  | BinOp (e1, op, e2) ->
  ...
  | Array es ->
      Printf.sprintf "[%s]"
        (String.concat ", "
           (map
              (fun e ->
                match e with None -> "" | Some e -> Printf.sprintf "%s" (f e)
                )
              es))
...
and string_of_catch_finally with_annot catch finally =
  ...
  | None -> ""
  | Some (v, e) ->
      Printf.sprintf "catch (%s) %s" v (string_of_exp with_annot e) )
  ^
  match finally with
  | None -> ""
